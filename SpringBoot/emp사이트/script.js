...
$.ajax({
    url : "http://localhost:8080/api/v1/emp/statistics",//백엔드 경로
    type : 'GET', //HTTP 메소드
...
var jsonData = {
    ...
    "comm" : comm}
$.ajax({
    url : "http://localhost:8080/api/v1/emp",//백엔드 경로
    type : 'POST', //HTTP 메소드
...
function getEmp(pageNum){
    $.ajax({
        url : "http://192.168.0.43:8080/api/v1/emp?page="+pageNum,//백엔드 경로
        type : 'GET', //HTTP 메소드
        datatype : 'Json', //프론트에서 백엔드 결과를 Json으로 받겠다.
        success : function(response){ //요청이 성공하면!
            //배열하고 for문은 친구!!
...
function getEmpByEmpno(empno){  
    ...
    $.ajax({
    url : "http://localhost:8080/api/v1/emp/empno/"+empno,//백엔드 경로
    type : 'GET', //HTTP 메소드
    datatype : 'Json', //프론트에서 백엔드 결과를 Json으로 받겠다.
    success : function(response){ //요청이 성공하면!
        console.log(response);//결과확인
